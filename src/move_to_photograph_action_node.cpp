...
{
public:
    MoveToPhotograph() : plansys2::ActionExecutorClient("move_to_photograph", 500ms)
    {
        ...
        odom_sub_ = this->create_subscription<nav_msgs::msg::Odometry>(
            "/odom", 10, std::bind(&MoveToPhotograph::odom_callback, this, std::placeholders::_1));
...
private:
    // function to update the current coordinated of the target wrt the robot
   void detection_callback(const aruco_opencv_msgs::msg::ArucoDetection::SharedPtr msg)
{
    for (const auto &marker : msg->markers)
    {
        // checking is the marker seen is the list of the targt marker
        bool is_in_list = std::any_of(targets_list.begin(), targets_list.end(),
            [&marker](const MarkerData &target) {
                return target.id == (long)marker.marker_id;
            });
        ...
        try
        {
            ...
            auto tf = tf_buffer_->lookupTransform(
                ...
                marker_frame,
                tf2::TimePointZero);
            this->live_marker_x_ = tf.transform.translation.x;
            this->live_marker_y_ = tf.transform.translation.y;
            this->marker_visible_ = true;
        }
        catch (const tf2::TransformException &ex)
        {
            RCLCPP_WARN(get_logger(), "TF failed for ID %d: %s", marker.marker_id, ex.what());
    ...
    std::string get_ws_path()
    {
        try
        {
            std::string pkg_path = ament_index_cpp::get_package_share_directory("plansys_interface");
            fs::path p(pkg_path);
    ...
    // function to get the markers seen and take the first to go to wp
    void load_detected_markers()
    {
        ...
        std::vector<MarkerData> all_markers;
        std::string line;
        MarkerData m;
        int fields_found = 0;
        ...
        std::string target_name = all_markers[0].name;
        this->targets_list.clear();

        // take only the markers with the same name
        for (const auto &marker : all_markers)
        {
            if (marker.name == target_name)
            {
                this->targets_list.push_back(marker);
            }
        }
        if (!targets_list.empty())
        {
            this->marker_target = targets_list[0];
            RCLCPP_INFO(get_logger(), "loaded %zu marker with name: %s", targets_list.size(), target_name.c_str());
        }
    }
    // function to remove the markers with the same name as the processed one
    void remove_from_yaml()
    {
        // get path
        std::string file_path = get_ws_path() + "/points_detected/detected_markers.yaml";
        std::vector<MarkerData> remaining_on_disk;

        std::string name_to_remove = this->marker_target.name;
        // get markers from yaml file
        std::ifstream infile(file_path);
        if (infile.is_open())
        {
            std::string line;
            ...
            while (std::getline(infile, line))
            {
                ...
                if (fields == 7) // removing markers with same name as target
                {
                    if (m.name != name_to_remove)
                    {
                        remaining_on_disk.push_back(m);
                    }
        ...
        {
            outfile << "markers:\n";
            for (const auto &rm : remaining_on_disk)
            {
                outfile << "  - id: " << rm.id << "\n"
                        << "    name: " << rm.name << "\n"
                        << "    frame: " << rm.frame << "\n"
    ...
    void do_work() override
    {
        ...
        if (args.size() < 2)
        {
            finish(false, 0.0, "no sufficient arguments");
            return;
        }
        ...
        {
            finish(false, 0.0, "no amrker avaiable");
            return;
        }
        ...
        if (this->state_ == AlignState::NAVIGATE) // navigate to wp
        {
            if (!goal_sent_)
            {
        ...
        else if (this->state_ == AlignState::ALIGN) // aligning state
        {
            if (!this->marker_visible_) // if no marker was seen
            {
            ...
            {
                // P-control
                double dx = this->live_marker_x_;
                double dy = this->live_marker_y_;
    ...
    // function to save image of marker
    void save_img()
    {
        try

        {
            ...
            // publishing img on images_detected
            sensor_msgs::msg::Image::SharedPtr img_msg = cv_ptr->toImageMsg();
            img_msg->header = last_img_->header;
    ...
    std::vector<MarkerData> targets_list;
