...
private:
  ...
  {
    if (markers_detected_this_run_.size() < TARGET_MARKERS_COUNT) // detect as many markes as decided (2) per block
    {
      for (auto &marker : msg->markers)
      {
        // check if the id was already detected
        bool already_in_local_list = std::any_of(
            markers_detected_this_run_.begin(),
            markers_detected_this_run_.end(),
            [&](const MarkerData &m)
            { return m.id == marker.marker_id; });

        if (!already_in_local_list) // if marker was not already detected save the marker
        {
          try
          {
  ...
  void do_work() override
  {
    ...
    // retrive marker to reach
    this->marker_to = args[1];
    double gx, gy;

    // define goal wp to go based on the marker retrived
    ...
    if (!goal_sent_)
    {
      ...
      options.result_callback = [this](auto result)
      {
        ...
        if (!nav2_success_)
        {
          RCLCPP_ERROR(get_logger(), "Navigation to %s failed: %s", this->marker_to.c_str(), reason.c_str());
    ...
    if (this->nav2_done_) // finish true
    {
      save_in_yaml();
  ...
  // function to save the marker detected in the yaml file
  void save_in_yaml()
  {
    // retrive the path of yaml file
